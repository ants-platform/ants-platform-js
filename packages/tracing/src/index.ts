...
export type StartObservationOpts = StartObservationOptions & {
  ...
  asType?: AntsPlatformObservationType;
};
...
/**
 * Creates an OpenTelemetry span with the Ants Platform tracer.
 *
...
function createOtelSpan(params: {
  ...
  return getAntsPlatformTracer().startSpan(
    params.name,
...
/**
 * Creates and starts a new Ants Platform observation with automatic TypeScript type inference.
 *
 * This is the primary method for creating observations in Ants Platform. It supports multiple
...
export function startObservation(
  ...
  switch (asType) {
    case "generation":
      return new AntsPlatformGeneration({
        otelSpan,
        attributes,
      });

    case "embedding":
      return new AntsPlatformEmbedding({
        otelSpan,
        attributes,
      });

    case "agent":
      return new AntsPlatformAgent({
        otelSpan,
        attributes,
      });

    case "tool":
      return new AntsPlatformTool({
        otelSpan,
        attributes,
      });

    case "chain":
      return new AntsPlatformChain({
        otelSpan,
        attributes,
      });

    case "retriever":
      return new AntsPlatformRetriever({
        otelSpan,
        attributes,
      });

    case "evaluator":
      return new AntsPlatformEvaluator({
        otelSpan,
        attributes,
      });

    case "guardrail":
      return new AntsPlatformGuardrail({
        otelSpan,
        attributes,
      });
    ...
    case "event": {
      ...
      return new AntsPlatformEvent({
        otelSpan,
        attributes: attributes as AntsPlatformEventAttributes,
        timestamp,
      });
    }
    case "span":
    default:
      return new AntsPlatformSpan({
        otelSpan,
        attributes: attributes as AntsPlatformSpanAttributes,
      });
  }
}
...
export function startActiveObservation<
  F extends (generation: AntsPlatformGeneration) => unknown,
>(
  name: string,
  fn: F,
...
export function startActiveObservation<
  F extends (embedding: AntsPlatformEmbedding) => unknown,
>(
  name: string,
  fn: F,
...
export function startActiveObservation<
  F extends (agent: AntsPlatformAgent) => unknown,
>(
  name: string,
  fn: F,
...
export function startActiveObservation<
  F extends (tool: AntsPlatformTool) => unknown,
>(
  name: string,
  fn: F,
...
export function startActiveObservation<
  F extends (chain: AntsPlatformChain) => unknown,
>(
  name: string,
  fn: F,
...
export function startActiveObservation<
  F extends (retriever: AntsPlatformRetriever) => unknown,
>(
  name: string,
  fn: F,
...
export function startActiveObservation<
  F extends (evaluator: AntsPlatformEvaluator) => unknown,
>(
  name: string,
  fn: F,
...
export function startActiveObservation<
  F extends (guardrail: AntsPlatformGuardrail) => unknown,
>(
  name: string,
  fn: F,
...
export function startActiveObservation<
  F extends (span: AntsPlatformSpan) => unknown,
>(
  name: string,
  fn: F,
 ...
 * @example
 * ```typescript
 * import { startActiveObservation } from '@antsplatform/tracing';
 *
 * // Span for general operations (default)
 ...
 */
export function startActiveObservation<
  F extends (observation: AntsPlatformSpan | AntsPlatformGeneration) => unknown,
>(name: string, fn: F, options?: StartActiveObservationOpts): ReturnType<F> {
  const { asType = "span", ...observationOptions } = options || {};

  return getAntsPlatformTracer().startActiveSpan(
    name,
    ...
    (span) => {
      try {
        let observation: AntsPlatformObservation;

        switch (asType) {
          case "generation":
            observation = new AntsPlatformGeneration({
              otelSpan: span,
            });
            break;

          case "embedding":
            observation = new AntsPlatformEmbedding({
              otelSpan: span,
            });
            break;

          case "agent":
            observation = new AntsPlatformAgent({
              otelSpan: span,
            });
            break;

          case "tool":
            observation = new AntsPlatformTool({
              otelSpan: span,
            });
            break;

          case "chain":
            observation = new AntsPlatformChain({
              otelSpan: span,
            });
            break;

          case "retriever":
            observation = new AntsPlatformRetriever({
              otelSpan: span,
            });
            break;

          case "evaluator":
            observation = new AntsPlatformEvaluator({
              otelSpan: span,
            });
            break;

          case "guardrail":
            observation = new AntsPlatformGuardrail({
              otelSpan: span,
            });
            break;
          ...
          case "event": {
            const timestamp = observationOptions?.startTime ?? new Date();
            observation = new AntsPlatformEvent({
              otelSpan: span,
              timestamp,
            });
            break;
          ...
          case "span":
          default:
            observation = new AntsPlatformSpan({
              otelSpan: span,
            });
        }
 ...
 * @example
 * ```typescript
 * import { updateActiveTrace } from '@antsplatform/tracing';
 *
 ...
 * @public
 */
export function updateActiveTrace(attributes: AntsPlatformTraceAttributes) {
 ...
 * and updates it with AntsPlatform-specific attributes. It supports all observation types
 ...
 * @example
 * ```typescript
 * import { updateActiveObservation, startActiveObservation } from '@antsplatform/tracing';
 *
...
export function updateActiveObservation(
  ...
  if (!options?.asType) {
    otelAttributes[AntsPlatformOtelSpanAttributes.OBSERVATION_TYPE] = undefined;
  }
...
/**
 * Decorator function that automatically wraps any function with AntsPlatform observability.
 *
 ...
 * @example
 * ```typescript
 * import { observe } from '@antsplatform/tracing';
 *
 ...
 *               non-W3C compliant IDs with AntsPlatform trace IDs. This allows you to later
 *               have a method available for scoring the AntsPlatform trace given only the
 ...
 * });
 *
 * // Correlating external IDs with AntsPlatform traces
