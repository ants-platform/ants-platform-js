...
function parseLogLevelFromEnv(): LogLevel | undefined {
  if (typeof process === "object" && "env" in process) {
    const envValue = getEnv("ANTS_PLATFORM_LOG_LEVEL");
 ...
 * @example
 * ```typescript
 * const logger = new Logger({ level: LogLevel.DEBUG, prefix: 'AntsPlatform SDK' });
...
export class Logger {
  ...
  private formatMessage(level: string, message: string): string {
    const timestamp = this.config.enableTimestamp
      ...
      : "";
    const prefix = this.config.prefix || "[AntsPlatform SDK]";
 ...
 * @example
 * ```typescript
 * import { logger } from '@antsplatform/core';
 *
