...
/**
 * Parameters for creating a AntsPlatformMedia instance.
 *
 ...
 * @public
 */
export type AntsPlatformMediaParams =
  | {
...
/**
 * A class for wrapping media objects for upload to AntsPlatform.
 *
 * This class handles the preparation and formatting of media content for AntsPlatform,
 ...
 * const media1 = new AntsPlatformMedia({
 *   source: "base64_data_uri",
 ...
 *
 * // From raw bytes
 * const media2 = new AntsPlatformMedia({
 *   source: "bytes",
 ...
 * @public
 */
export class AntsPlatformMedia {
  _contentBytes?: Uint8Array;
  _contentType?: MediaContentType;
  _source?: string;

  /**
   * Creates a new AntsPlatformMedia instance.
   *
   ...
   * ```
   */
  constructor(params: AntsPlatformMediaParams) {
    const { source } = params;
  ...
  async getSha256Hash(): Promise<string | undefined> {
    ...
    try {
      const hash = await crypto.subtle.digest(
        "SHA-256",
        this._contentBytes.buffer as ArrayBuffer,
      );

      return bytesToBase64(new Uint8Array(hash));
    } catch (error) {
      getGlobalLogger().warn(
        "[AntsPlatform] Failed to generate SHA-256 hash for media content:",
        error,
      );
   ...
   *
   * The tag format is: `@@@antsPlatformMedia:type=<contentType>|id=<mediaId>|source=<source>@@@`
   * This tag can be embedded in trace attributes and will be replaced with actual
   * media content when the trace is viewed in AntsPlatform.
   *
   ...
   * // "@@@antsPlatformMedia:type=image/png|id=A1B2C3D4E5F6G7H8I9J0K1|source=base64_data_uri@@@"
   * ```
   */
  async getTag(): Promise<string | null> {
    ...
    return `@@@antsPlatformMedia:type=${this._contentType}|id=${id}|source=${this._source}@@@`;
  }
