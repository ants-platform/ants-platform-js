...
import { AntsPlatformPromptCache } from "./promptCache.js";
import {
  ChatPromptClient,
  TextPromptClient,
  AntsPlatformPromptClient,
...
/**
 * Manager for prompt operations in Ants Platform.
 *
...
export class PromptManager {
  private cache: AntsPlatformPromptCache;
  ...
  constructor(params: { apiClient: AntsPlatformAPIClient }) {
    ...
    this.cache = new AntsPlatformPromptCache();
  }
  ...
  /**
   * Creates a new prompt in Ants Platform.
   *
   ...
   * const textPrompt = await antsPlatform.prompt.create({
   *   name: "greeting",
   *   prompt: "Hello {{name}}!",
   *   type: "text"
   * });
   ...
   * const chatPrompt = await antsPlatform.prompt.create({
   *   name: "conversation",
   *   type: "chat",
   *   prompt: [
  ...
  async create(
    body:
      ...
      | CreateChatPromptBodyWithPlaceholders,
  ): Promise<AntsPlatformPromptClient> {
   ...
   * const v2Prompt = await antsPlatform.prompt.get("my-prompt", {
   *   version: 2
   * });
   *
   * // Get with label filter
   * const prodPrompt = await antsPlatform.prompt.get("my-prompt", {
   *   label: "production"
   * });
   *
   * // Get with fallback
   * const promptWithFallback = await antsPlatform.prompt.get("my-prompt", {
   *   type: "text",
   *   fallback: "Hello {{name}}!"
   * });
   * ```
  ...
  private async fetchPromptAndUpdateCache(params: {
    ...
    try {
      ...
      let prompt: AntsPlatformPromptClient;
      if (data.type === "chat") {
