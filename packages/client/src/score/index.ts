...
/**
 * Manager for creating and batching score events in AntsPlatform.
 *
...
export class ScoreManager {
  ...
  constructor(params: { apiClient: AntsPlatformAPIClient }) {
    ...
    const envFlushAtCount = getEnv("ANTS_PLATFORM_FLUSH_AT");
    const envFlushIntervalSeconds = getEnv("ANTS_PLATFORM_FLUSH_INTERVAL");
  ...
  public create(data: ScoreBody): void {
    const scoreData: ScoreBody = {
      ...
      environment:
        data.environment ?? getEnv("ANTS_PLATFORM_TRACING_ENVIRONMENT"),
    };
   ...
   * @example
   * ```typescript
   * import { startSpan } from '@antsplatform/tracing';
   *
   * const span = startSpan({ name: "my-operation" });
   * antsPlatform.score.observation(
   *   { otelSpan: span },
   ...
   * @example
   * ```typescript
   * import { startActiveSpan } from '@antsplatform/tracing';
   *
   * startActiveSpan({ name: "my-operation" }, (span) => {
   *   // Inside the active span
   *   antsPlatform.score.activeObservation({
   *     name: "relevance",
   *     value: 0.95
   *   });
   * });
   ...
   * startActiveSpan({ name: "my-operation" }, (span) => {
   *   // Inside the active span
   *   antsPlatform.score.activeTrace({
  ...
  /**
   * Flushes all pending score events to the AntsPlatform API.
   *
   ...
   * await antsPlatform.score.flush(); // Ensures the score is sent immediately
   * ```
   */
  public async flush() {
   ...
   * all score data is sent to AntsPlatform.
   *
   ...
   * await antsPlatform.score.shutdown();
   * ```
   */
