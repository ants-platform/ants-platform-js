 ...
 * @public
 */
export type AntsPlatformMediaResolveMediaReferencesParams<T> = {
...
/**
 * Manager for media operations in AntsPlatform.
 *
...
export class MediaManager {
   ...
   * in the format "@@@antsPlatformMedia:...@@@". When found, it fetches the actual media
   * content from AntsPlatform and replaces the reference string with a base64 data URI.
   *
   ...
   * ```typescript
   * const obj = {
   *   image: "@@@antsPlatformMedia:type=image/jpeg|id=123|source=bytes@@@",
   *   nested: {
   *     pdf: "@@@antsPlatformMedia:type=application/pdf|id=456|source=bytes@@@"
   *   }
   * };
   *
   * const result = await antsPlatform.media.resolveReferences({
   *   obj,
   *   resolveWith: "base64DataUri"
   * });
   *
  ...
  public async resolveReferences<T>(
    params: AntsPlatformMediaResolveMediaReferencesParams<T>,
  ): Promise<T> {
    ...
    const traverse = async <T>(obj: T, depth: number): Promise<T> => {
      ...
      if (typeof obj === "string") {
        const regex = /@@@antsPlatformMedia:.+?@@@/g;
   ...
   *     "@@@antsPlatformMedia:type=image/jpeg|id=some-uuid|source=base64DataUri@@@"
   *
  ...
  public static parseReferenceString(
    ...
    const prefix = "@@@antsPlatformMedia:";
    const suffix = "@@@";

    if (!referenceString.startsWith(prefix)) {
      throw new Error(
        "Reference string does not start with '@@@antsPlatformMedia:type='",
      );
    }
