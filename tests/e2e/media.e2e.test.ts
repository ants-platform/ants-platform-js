...
import { resetGlobalLogger, AntsPlatformMedia } from "@ants-platform/core";
...
describe("Media E2E Tests", () => {
  ...
  describe("Media Reference Replacement", () => {
    it("replace media reference string in object when using base 64 data URIs", async () => {
      ...
      const base64DataUri = new AntsPlatformMedia({
        source: "bytes",
        contentBytes: Buffer.from(mockAudioBytes),
        contentType: "audio/wav",
      }).base64DataUri;

      // Create span with AntsPlatformMedia in metadata (should use toJSON to convert to media reference)
      ...
      // Fetch the trace from AntsPlatform server
      ...
      if (observation.input) {
        ...
        // Should contain AntsPlatform media reference in input
        expect(inputStr).toMatch(
          /@@@antsPlatformMedia:type=audio\/wav\|id=.+\|source=base64_data_uri@@@/,
        );
      ...
      // Should contain AntsPlatform media reference in metadata
        ...
        if (resolvedSpanObs?.input?.audioData) {
          ...
          const span3 = startObservation(span3Name, {
            input: {
              ...
              audioData: new AntsPlatformMedia({
    ...
    it("replace media reference string in object when using AntsPlatformMedia objects", async () => {
      const testId = nanoid(8);
      const spanName = `media-processing-span-${testId}`;
