...
import { observeOpenAI } from "@ants-platform/openai";
...
describe("OpenAI integration E2E tests", () => {
  ...
  it("should trace OpenAI Chat Completion Create ", async () => {
    ...
    const config = {
      ...
      userId: "my-user",
      tags: ["tag1", "tag2"],
      antsPlatformPrompt: await antsPlatformClient.prompt.get(promptName),
    ...
    const traces = await antsPlatformClient.api.trace.list({ name: traceName });
    ...
    const observations = await antsPlatformClient.api.observations.getMany({
      traceId: trace.id,
    });
  ...
  it("should trace chat completion with streaming", async () => {
    ...
    const stream = await wrappedOpenAI.chat.completions.create({
      ...
      model: "gpt-3.5-turbo",
      stream: true,
      user: "antsPlatform-user@gmail.com",
      max_tokens: 300,
    });
    ...
    const traces = await antsPlatformClient.api.trace.list({
      name: generationName,
    });
    expect(traces.data.length).toBe(1);
  ...
  it("should trace with extra wrapper params", async () => {
    ...
    const wrappedOpenAI = observeOpenAI(new OpenAI(), {
      ...
      tags: ["hello", "World"],
      sessionId: "AntsPlatform",
      userId: "AntsPlatformUser",
    });
    ...
    expect(trace.sessionId).toBe("AntsPlatform");
    expect(trace.userId).toBeDefined();
    expect(trace.userId).toBe("AntsPlatformUser");
