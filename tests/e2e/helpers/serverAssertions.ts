...
export class ServerAssertions {
  ...
  constructor() {
    this.baseUrl = getEnv("ANTS_PLATFORM_BASE_URL") || "http://localhost:3000";

    const publicKey = getEnv("ANTS_PLATFORM_PUBLIC_KEY");
    const secretKey = getEnv("ANTS_PLATFORM_SECRET_KEY");

    if (!publicKey || !secretKey) {
      throw new Error(
        "ANTS_PLATFORM_PUBLIC_KEY and ANTS_PLATFORM_SECRET_KEY must be set for E2E tests",
      );
    }
