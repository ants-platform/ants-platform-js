...
import { startActiveObservation } from "@ants-platform/tracing";
import { ChatOpenAI } from "@langchain/openai";
import { ChatPromptTemplate } from "@langchain/core/prompts";
import { CallbackHandler } from "@ants-platform/langchain";
import { DynamicTool } from "@langchain/core/tools";
import { StateGraph, MessagesAnnotation } from "@langchain/langgraph";
import { configureGlobalLogger } from "@ants-platform/core";

describe("Langchain integration E2E tests", () => {
  let antsPlatformClient: AntsPlatformClient;
  let testEnv: ServerTestEnvironment;

  beforeEach(async () => {
    configureGlobalLogger({ level: 0 });
    testEnv = await setupServerTestEnvironment();
    antsPlatformClient = new AntsPlatformClient();
  });
  ...
  it("should trace a chain", async () => {
    ...
    const trace = await antsPlatformClient.api.trace.get(traceId!);
  ...
  it("should link a antsPlatform prompt", async () => {
    const testConfig = {
      runName: "Test simple chain:" + nanoid(),
      sessionId: "my-session",
      userId: "my-user",
    ...
    await antsPlatformClient.prompt.create({
      name: jokePromptName,
      type: "chat",
    ...
    // Fetch prompts
    const antsPlatformJokePrompt = await antsPlatformClient.prompt.get(
      jokePromptName,
      {
        type: "chat",
      },
    ...
    const langchainJokePrompt = ChatPromptTemplate.fromMessages(
      antsPlatformJokePrompt.getLangchainPrompt(),
    ).withConfig({
      metadata: { antsPlatformPrompt: antsPlatformJokePrompt },
    });
    ...
    expect(generation!.completionTokens).toBeDefined();
    expect(generation!.output.content).toContain(result.content);
    expect(generation!.promptName).toBe(antsPlatformJokePrompt.name);
    expect(generation!.promptVersion).toBe(antsPlatformJokePrompt.version);
  });
    ...
    const result = await chain.invoke(
      { query: testConfig.query },
      {
        ...
        metadata: {
          ...
          antsPlatformSessionId: testConfig.sessionId,
          antsPlatformUserId: testConfig.userId,
        },
        tags: testConfig.tags,
      },
    );
