...
describe("Vercel AI SDK integration E2E tests", () => {
  ...
  it("should trace a streamText call with linked prompts", async () => {
    ...
    const fetchedPrompt = await antsPlatformClient.prompt.get(promptName);
      ...
      async (span) => {
        const stream = streamText({
          ...
          experimental_telemetry: {
            ...
            functionId,
            metadata: {
              antsPlatformPrompt: fetchedPrompt.toJSON(),
              userId,
              sessionId,
              tags,
              ...metadata,
  ...
  it("should trace a call with file attachment", async () => {
    ...
    expect(inputStr).toMatch(
      /@@@antsPlatformMedia:type=application\/pdf\|id=.+\|source=bytes@@@/,
    );
  ...
  it("should trace a call with image", async () => {
    ...
    expect(inputStr).toMatch(
      /@@@antsPlatformMedia:type=image\/jpeg\|id=.+\|source=bytes@@@/,
    );
