...
import { AntsPlatformClient } from "@ants-platform/client";
import {
  ChatPromptClient,
  TextPromptClient,
  ChatMessageType,
} from "@ants-platform/client";
import type {
  ChatMessage,
  ChatMessageWithPlaceholders,
} from "@ants-platform/core";

describe("AntsPlatform Prompts E2E", () => {
  let antsPlatform: AntsPlatformClient;

  beforeEach(async () => {
    antsPlatform = new AntsPlatformClient();
  });

  describe("prompts", () => {
    it("should create a prompt", async () => {
      const createdPrompt = await antsPlatform.prompt.create({
        name: "test-prompt",
    ...
    it("should get a prompt by name only", async () => {
      ...
      await antsPlatform.prompt.create({
      ...
      // Then retrieve it
      const retrievedPrompt = await antsPlatform.prompt.get("test-get-prompt");
    ...
    it("should get a prompt by name only with getPrompt", async () => {
      ...
      // Then retrieve it
      const retrievedPrompt = await antsPlatform.getPrompt("test-get-prompt");
    ...
    it("should get a prompt with name and version", async () => {
      ...
      const retrievedPrompt = await antsPlatform.prompt.get(
    ...
    it("should retry if custom request timeout is exceeded", async () => {
      ...
      await expect(
        antsPlatform.prompt.get("non-existent-prompt", {
          fetchTimeoutMs: 300,
          maxRetries: 2,
        }),
      ).rejects.toThrow();
    ...
    it("should fetch and cache a prompt when not in cache", async () => {
      ...
      const result = await antsPlatform.prompt.get("test-cache-prompt");
    ...
    it("should return cached prompt if not expired", async () => {
      ...
      const fetchSpy = vi.spyOn(antsPlatform.prompt.apiClient.prompts, "get");

      // Get it twice - second call should use cache
      const result1 = await antsPlatform.prompt.get("test-cached-prompt");
      const result2 = await antsPlatform.prompt.get("test-cached-prompt");
    ...
    it("should return cached prompt if not expired according to custom TTL", async () => {
      ...
      const result1 = await antsPlatform.prompt.get("test-custom-ttl", {
        cacheTtlSeconds,
      });
      const result2 = await antsPlatform.prompt.get("test-custom-ttl", {
        cacheTtlSeconds,
      });
    ...
    it("should always fetch latest version of prompt if cacheTtlSeconds is 0", async () => {
      ...
      const result1 = await antsPlatform.prompt.get("test-no-cache", {
        cacheTtlSeconds: 0,
      });
      const result2 = await antsPlatform.prompt.get("test-no-cache", {
        cacheTtlSeconds: 0,
      });
    ...
    it("should not make network call when prompt is already cached", async () => {
      ...
      await antsPlatform.prompt.get("test-already-cached");

      // Now spy on API calls for subsequent requests
      const fetchSpy = vi.spyOn(antsPlatform.prompt.apiClient.prompts, "get");

      // This call should use cache and make no network request
      const cachedResult = await antsPlatform.prompt.get("test-already-cached");
    ...
    describe("update method", () => {
      it("should update prompt labels successfully", async () => {
        ...
        const initialPrompt = await antsPlatform.prompt.create({
        ...
        // Update the labels
        const updatedPrompt = await antsPlatform.prompt.update({
          name: "test-update-prompt",
      ...
      it("should invalidate cache after update", async () => {
        ...
        const cachedPrompt = await antsPlatform.prompt.get(
          "test-cache-invalidation",
        );
        expect(cachedPrompt.labels).toContain("production");

        // Update the prompt labels
        await antsPlatform.prompt.update({
          name: "test-cache-invalidation",
          version: initialPrompt.version,
        ...
        const freshPrompt = await antsPlatform.prompt.get(
          "test-cache-invalidation",
        );
    ...
    describe("fallback behavior", () => {
      it("should use text fallback when prompt not found", async () => {
        const result = await antsPlatform.prompt.get(
      ...
      it("should prefer server prompt over fallback when available", async () => {
        ...
        const result = await antsPlatform.prompt.get("real-vs-fallback", {
      ...
      it("should throw error when no fallback provided and prompt not found", async () => {
        await expect(
          antsPlatform.prompt.get("definitely-does-not-exist"),
        ).rejects.toThrow();
      });
    });
    ...
    describe("server-side data integrity", () => {
      it("should store and retrieve all prompt fields correctly", async () => {
        ...
        const serverPrompt = await antsPlatform.api.prompts.get(
      ...
      it("should maintain data consistency after updates", async () => {
        ...
        const serverPrompt =
          await antsPlatform.api.prompts.get("consistency-test");
    ...
    describe("custom label handling", () => {
      it("should create and retrieve prompts with custom labels", async () => {
        ...
        const result = await antsPlatform.prompt.get("custom-label-test", {
          label: "staging",
        });
      ...
      it("should handle prompts without production label", async () => {
        ...
        await expect(
          antsPlatform.prompt.get("no-production-label"),
        ).rejects.toThrow();

        // Should find with explicit label
        const result = await antsPlatform.prompt.get("no-production-label", {
          label: "development",
        });
      ...
      it("should prioritize version over label in cache key", async () => {
        const prompt1 = await antsPlatform.prompt.create({
        ...
        const resultByVersion = await antsPlatform.prompt.get(
        ...
        const resultByLabel = await antsPlatform.prompt.get(
          "version-priority-test",
          {
            label: "staging",
          },
    ...
    describe("concurrent cache behavior", () => {
      it("should prevent multiple concurrent cache refreshes", async () => {
        ...
        await antsPlatform.prompt.get("concurrent-test");

        // Mock the cache to be expired
        const cacheKey = antsPlatform.prompt["cache"].createKey({
          name: "concurrent-test",
        });
        const cachedItem =
          antsPlatform.prompt["cache"].getIncludingExpired(cacheKey);
        if (cachedItem) {
        ...
        const promises = [
          antsPlatform.prompt.get("concurrent-test"),
          antsPlatform.prompt.get("concurrent-test"),
          antsPlatform.prompt.get("concurrent-test"),
          antsPlatform.prompt.get("concurrent-test"),
        ];
    ...
    describe("comprehensive error scenarios", () => {
      it("should handle network timeout gracefully", async () => {
        ...
        await expect(
          antsPlatform.prompt.get("timeout-test", {
      ...
      it("should handle invalid version numbers", async () => {
        await expect(
          antsPlatform.prompt.get("test", { version: -1 }),
        ).rejects.toThrow();
      });
      ...
      it("should handle special characters in prompt names", async () => {
        ...
        for (const name of specialNames) {
          try {
            ...
            const retrieved = await antsPlatform.prompt.get(name);
      ...
      it("should actually retry the specified number of times on failure", async () => {
        ...
        try {
          await antsPlatform.prompt.get("test-prompt-retry", {
            maxRetries: 3,
          });
        } catch (error) {
      ...
      it("should respect maxRetries=0 and not retry", async () => {
        ...
        try {
          await antsPlatform.prompt.get("non-existent-no-retry-test", {
            maxRetries: 0,
            fetchTimeoutMs: 100,
          });
        } catch (error) {
      ...
      it("should handle maxRetries with successful retry", async () => {
        ...
        try {
          const result = await antsPlatform.prompt.get(testPromptName, {
            maxRetries: 5,
      ...
      it("should handle very large prompt names", async () => {
        ...
        try {
          ...
          const retrieved = await antsPlatform.prompt.get(veryLongName);
    ...
    it("should correctly get langchain prompt format", async () => {
      ...
      for (let i = 0; i < testPrompts.length; i++) {
        ...
        // Create a new prompt
        const antsPlatformPrompt = new TextPromptClient({
          name: `test_${i}`,
          version: 1,
          prompt: testPrompt,
          type: "text",
        ...
        const langchainPrompt = ChatPromptTemplate.fromTemplate(
          antsPlatformPrompt.getLangchainPrompt(),
        );
    ...
    it("should correctly get langchain prompt format for chats", async () => {
      ...
      for (let i = 0; i < testPrompts.length; i++) {
        ...
        // Create a new prompt
        const antsPlatformPrompt = new ChatPromptClient({
          name: `test_${i}`,
          version: 1,
