...
describe("Server Export E2E Tests", () => {
  ...
  it("should export span with nested generation to AntsPlatform server", async () => {
    ...
    // Fetch the trace from AntsPlatform server and verify
    ...
    console.log(
      "✅ E2E test passed: Trace exported successfully to AntsPlatform server",
    );
    console.log(`📊 Trace ID: ${trace.id}`);
    console.log(`📈 Observations: ${trace.observations.length}`);
  });

  it("should export startActiveObservation with nested startActiveObservation generation to AntsPlatform server", async () => {
    const testId = nanoid(8);
  ...
  it("should export observe wrapper with interoperability to AntsPlatform server", async () => {
    const testId = nanoid(8);
  ...
  it("should export spans media handling to AntsPlatform server", async () => {
    const testId = nanoid(8);
...
Both media items were successfully processed. The image is a minimal transparent PNG and the audio is a brief silent WAV file. Processing completed in 20ms with high confidence scores.`,
    ...
    if (generationObs.input) {
      ...
      if (
        inputStr.includes("@@@antsPlatformMedia:") ||
