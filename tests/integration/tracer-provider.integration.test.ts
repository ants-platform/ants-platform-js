import { AntsPlatformSpanProcessor } from "@ants-platform/otel";
import {
...
describe("Tracer Provider Isolation Integration Tests", () => {
  ...
  beforeEach(async () => {
    ...
    const isolatedSpanProcessor = new AntsPlatformSpanProcessor({
    ...
    setAntsPlatformTracerProvider(isolatedProvider);
  });

  afterEach(async () => {
    // Cleanup isolated provider
    setAntsPlatformTracerProvider(null);
  ...
  describe("Multiple isolated providers", () => {
    it("should handle multiple isolated providers independently", async () => {
      ...
      // Test first provider
      setAntsPlatformTracerProvider(firstProvider);
      let tracer = getAntsPlatformTracer();
      let span = tracer.startSpan("first-provider-span");
      span.end();

      // Test second provider
      setAntsPlatformTracerProvider(secondProvider);
      tracer = getAntsPlatformTracer();
  ...
  describe("Error scenarios", () => {
    ...
    it("should maintain global context even when isolated provider has issues", async () => {
      ...
      await context.with(globalContext, async () => {
        const isolatedTracer = getAntsPlatformTracer();
